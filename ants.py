...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                # 用can_contain 來判定已經排除掉被包的那方也是container了，所以只可能有一方是container
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and not insect.is_imposter:
                return
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if self.is_scared:
            if self.place.entrance is not colony.hive:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def nearest_bees_start_form_place(place, distance=0):
            if place is hive:
                return 
            if place.bees and (
                                (self.max_range == None and self.min_range == None) 
                                or
                                (self.min_range and distance >= self.min_range)
                                or
                                (self.max_range and distance <= self.max_range)):
                return place.bees

            return nearest_bees_start_form_place(place.entrance, distance + 1)

        return random_or_none(nearest_bees_start_form_place(self.place))
...
class HungryAnt(Ant):
    ...
    def __init__(self, armor=1):
        ...
        self.digesting = 0 # 剩下消化輪數
        self.time_to_digest = 3 # 預設消化一次需要幾輪
        # END Problem 6
    ...
    def action(self, colony):
        ...
        if not self.digesting:
            poor_bee = random_or_none(self.place.bees)
            self.eat_bee(poor_bee)
        else:
            self.digesting -= 1
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(armor)
        self.is_imposter = QueenAnt.is_imposter
        if not QueenAnt.is_imposter:
            QueenAnt.is_imposter = True
        self.buffed_ants = []
        # END Problem 13
    def remark_buffed_ant(self, ant):
        self.buffed_ants.append(ant)
    
    def buff(self, ant):
        # only buff didn't be buffed ant
        if ant not in self.buffed_ants:
            ant.damage *= 2
            self.remark_buffed_ant(ant)

    def buff_single_place_ants(self, place):
        # buff all ant in single place
        current_ant = place.ant
        # is container or not
        if current_ant.is_container:
            
            # which has damage
            if hasattr(current_ant, 'damage'):
                self.buff(current_ant)
            
            # which has contained ant
            contained_ant = current_ant.contained_ant
            if contained_ant:
                self.buff(current_ant.contained_ant)
        else:
            self.buff(current_ant)
    
    def buff_to_behide(self, place):
        # buff all all ant behind queen
        if place is None:
            return

        if place.ant:
            self.buff_single_place_ants(place)
        self.buff_to_behide(place.exit)

    def clean_buffed_ant_but_dead_in_remark(self):
         for a in self.buffed_ants[:]:
            if a.armor <= 0:
                self.buffed_ants.remove(a)
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            return
        super().action(colony)
        self.clean_buffed_ant_but_dead_in_remark()
        self.buff_to_behide(self.place.exit)
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    def effect_action(colony):
        nonlocal duration
        if duration > 0:
            effect(origin_action, bee)(colony)
            duration -= 1
        else:
            origin_action(colony)
            bee.action = origin_action
